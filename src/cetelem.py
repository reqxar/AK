...
# Настройки
docStartDateBegin = '08.11.2021' # Дата начала периода
docStartDateEnd = '08.11.2021' # Дата окончания периода

file_quantity = 10 # Количество файлов
doc_quantity = 150 # Количество документов в файле

thread_count = 6

savePath = 'D:/out/cetelem/'
    ...
    for operation_day in Requisite.get_date_range(docStartDateBegin, docStartDateEnd):
        for i in range(round(file_quantity / thread_count)):
            ...
            for n in range(doc_quantity):
                ...
                AK.add('packNumber', 'bagSearch')
                AK.add('pageQuantity', '1')
                AK.add('department', 'ОСОП')
                AK.add('storeTerm', '5')
