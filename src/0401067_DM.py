...
# Настройки
docStartDateBegin = '08.07.2021' # Дата начала периода
docStartDateEnd = '08.07.2021' # Дата окончания периода
    ...
    for operation_day in Requisite.get_date_range(docStartDateBegin, docStartDateEnd):
        for i in range(round(file_quantity / thread_count)):
            ...
            for n in range(doc_quantity):
                ...
                AK.random_string('formCode', ['0401067_DM'])
                ...
                AK.add('payer', get_random_name())
                AK.add('Recipient', get_random_name())
                AK.add('postingFIO', get_random_name())
                AK.add('signOneStatus', 'OK')
                AK.add('signTwoStatus', 'OK')
